...
function line(config) {
    ...
    lineChart.setMakeChart(makeLineChart);

    function makeLineChart(selection) {
        selection.each(function(data) {
            ...
            var scalex = d3.scale.linear()
                  ...
                  .range([0, lineChart.width() - margin.left - margin.right]);

            var scaley = d3.scale.linear()
                  .domain([0, d3.max(data, lineChart.y())])
                  .range([lineChart.height() - margin.top - margin.bottom, 0]);
